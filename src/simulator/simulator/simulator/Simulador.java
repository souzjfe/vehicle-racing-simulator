...
public class Simulador {
   ...
   * contains it or not. A função pede para o usuário um id de vaículo e valida se
   * o array possui ele ou não
   * 
  ...
  /**
   * This function includes a vehicle in the array of vehicles A função insere um
   * veículo no array de veículos
   */
  public void includeVehicle() {
    ...
    do {
      ...
      if (amountVehicles + vechiclesLength > 20) {
        char option;
        do {
          // Pergunta se o usuário deseja incluir a quantidade de veículos restante no
          // array
          ...
          // Se o usuário escolher preencher o array, coloca n veículos no vetor, se não,
          // informa que não foi incluído nenhum veículo, se a opção for inválida, informa
          // o usuário
          if (option == 's') {
      ...
      // Se a quantidade informada for negativa, informa o usuário para informar de
      // novo
  ...
  /**
   * Fuels the tank of the vehicle Enche o tanque de combustível
   */
  ...
  /**
   * Makes a certain vehicle run Faz um determinado veículo correr
   */
  public void runVehicle() {
    ...
    System.out.println(vehicle.run() ? "O veículo foi movido com sucesso" : "Não foi possível movimentar o veículo. Verifique o IPVA ou a calibragem dos pneus");
  }

  /**
   * Moves all vehicles Move todos os veículos
   */
  public void runVehicles() {
    ...
    for (PopularCar vehicle : vehicles.getVehicles()) {
      ...
      if (vehicle != null) {
        ...
        if (!vehicle.run()) {
          System.out.printf("Não foi possívle movimentar o veículo de id %d. Verifique o IPVA ou a calibragem dos pneus\n", vehicle.getId());
        }
      }
    }
  ...
  /**
   * Decalibrates a wheel Descalibra uma roda
   */
  public void decalibrateWheel() {
    ...
    do {
      ...
      wheelIndex = scan.nextInt();
      // Chama o método para descalibrar a roda e informa o usuário se deu certo ou
      // nao
      if (wheelIndex == 4) {
        System.out
            .println(vehicle.decalibrateWheel() ? "Roda descalibrada com successo" : "Houve uma falha ao descalibrar!");
      } else {

        System.out.println(vehicle.decalibrateWheel(wheelIndex) ? "Roda descalibrada com successo"
            : "Houve uma falha ao descalibrar!");
      }
      if (wheelIndex != 0 && wheelIndex != 1 && wheelIndex != 2 && wheelIndex != 3 && wheelIndex != 4) {
        System.out.println("Informe uma opção válida");
      }
    } while (wheelIndex != 0 && wheelIndex != 1 && wheelIndex != 2 && wheelIndex != 3 && wheelIndex != 4);
  }

  /**
   * Calibrates a wheel Calibra uma roda
   */
  public void calibrateWheel() {
    // Faz o mesmo processo do método anterior, mas chama o método de calibrar a
    // roda no final
    int Id = requestVehicleId();
    PopularCar vehicle = vehicles.searchById(Id);
    int wheelIndex;
    do {
      ...
      wheelIndex = scan.nextInt();
      if (wheelIndex == 4) {
        System.out.println(vehicle.calibrateWheel() ? "Roda calibrada com successo" : "Houve uma falha ao calibrar!");
      } else {
  ...
  /**
   * Calibrates all wheels of all the vehicles Calibra todas as rodas de todos os
   * veículos
   */
  public void calibrateAllVeihiclesWheels() {
    // Percorre o array de veículos
    for (PopularCar vehicle : vehicles.getVehicles()) {
      // Calibra a roda de todos os veículos e infomra o usuário caso ocorra algum
      // erro
  ...
  /**
   * Lists all the informations about all the vehicles Lista todas as informações
  ...
  public void listVehicles() {
    ...
    if (vehiclesArray.length > 0) {
      ...
      for (int i = 0; i < vehiclesArray.length; i++) {
        System.out.printf("\n\nApresentando veículo de numero %d\n", i);
        System.out.println(vehiclesArray[i].toString());
      }
    } else {
  ...
  /**
   * Shows all the vehicles Mostra todos os veículos
   * 
   */
  public void showVehicles() {
    // Faz o mesmo processo da função anterior, mas chama a função de imprimir os
    // veículos na tela
    PopularCar[] vehiclesArray = vehicles.getVehicles();
    if (vehiclesArray.length > 0) {
      for (int i = 0; i < vehiclesArray.length; i++) {
